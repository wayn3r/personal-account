  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!token)
      throw this.handleError(
        'TOKEN_NOT_FOUND',
        'Token header not found, set x-token to a valid token string',
      )

    const tokenResult = await this.authRepository.verify(token)
    if (tokenResult.isFailure())
      throw this.handleError('TOKEN_INVALID', 'Token is not valid')

    const externalId = tokenResult.getOrThrow()

    const userResult = (await this.userRepository.findByExternalId(externalId)).flatMap(
      (userOrNull) =>
        userOrNull.validateIsPresent(
          () => new NotFound(DomainError.of('USER_NOT_FOUND')),
        ),
    )
    if (userResult.isFailure())
      throw this.handleError(userResult.getErrorOrThrow().code, 'User not found')
